...
from facefusion.ffmpeg import open_ffmpeg, pipe_frames_to_process, start_ffmpeg
...
# from facefusion.content_analyser import analyse_stream # Commented out import
	...
	with tqdm(desc = wording.get('streaming'), unit = 'frame', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
		with ThreadPoolExecutor(max_workers = state_manager.get_item('execution_thread_count')) as executor:
			...
			while webcam_capture and webcam_capture.isOpened():
				...
				# if analyse_stream(capture_frame, webcam_fps): # Commented out block
				# 	yield None
